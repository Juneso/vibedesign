...
// 온보딩 응답 저장 (BKT-271)
export function saveOnboarding({ preferredGenres, thinkStyle, stopPattern }) {
  const store = getStore();
  store.onboarding = { preferredGenres, thinkStyle, stopPattern, completedAt: new Date().toISOString() };
  _save(store);
  return store;
}

// T1 트리거 — 첫 기록 저장 직후 Apple 업그레이드 유도 (BKT-277)
export function shouldShowT1() {
  const store = getStore();
  return !store.t1Shown && store.records.length === 1;
}

export function markT1Shown() {
  const store = getStore();
  store.t1Shown = true;
  _save(store);
}
