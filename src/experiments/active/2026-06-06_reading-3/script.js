...
import { getStore, addRecord, resetStore, addBook } from './store.js';
...
root.addEventListener('click', e => {
  ...
  if (obGenre) obGenre.classList.toggle('selected');
});
...
function handleAction(action, el) {
  switch (action) {
    ...
    case 'ob-has-book': {
      root.querySelector('#ob-choice-wrap').style.display = 'none';
      root.querySelector('#ob-book-entry').style.display = 'flex';
      root.querySelector('#ob-book-title').focus();
      break;
    }
    case 'ob-no-book':
    case 'ob-finish':
    ...
    case 'save-capture': {
      ...
      if (text) addRecord({ text, book, author, memo, tags, createdAt: new Date().toISOString() });
