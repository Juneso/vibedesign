...
판단:
1) 후보 중 사실상 같은 개념이 있으면 → op="merge", targetId=그 id. (유사도 0.55+ 면 적극 병합)
2) 아니면 새 노드 → op="attach". 부모는: 이미 더 일반적인 상위개념 노드가 트리에 있으면 그 id(위계를 키워라). 적절한 상위개념이 없으면 targetId="${root.id}"(책 루트 = 새 가지). 목차 제목을 노드로 만들지 마라.
...
// ─── Phase 2: fan-out 재정리 (위생: 부모명 ≠ 자식명) ───────────
const FANOUT = 4;
for (const node of [...nodes.values()]) {
  const kids = childrenOf(node.id);
  if (kids.length <= FANOUT) continue;
  const prompt = `${book.toc.join(' · ')} 를 숨은 참고로 삼아, 부모 "${node.title}" 밑 개념들을 비슷한 것끼리 2~3개의 중간 상위개념으로 묶어라. 중간개념 이름은 자식 중 하나와 똑같이 짓지 마라(더 일반적인 이름). 묶을 필요 없으면 비워라.
...
출력 JSON: {"groups":[{"newParent":"중간개념명","childIds":["n?","n?"]}]}`;
  ...
  for (const g of out.groups || []) {
    ...
    const mid = addConcept(g.newParent, node.id, null, '재정리 중간개념');
    for (const cid of g.childIds) if (nodes.has(cid)) { nodes.get(cid).parentId = mid.id; nodes.get(cid).level = mid.level + 1; }
    log.push(`[재정리] "${node.title}" 밑 ${g.childIds.length}개 → 중간개념 "${g.newParent}"(${mid.id})`);
  }
}
