...
if (process.env.ALIAS === '1') {
  ...
  const llm = claudeCliTransport({ model: process.env.MODEL || 'claude-haiku-4-5-20251001' });
  const names = new Set();
  ...
  try {
    ...
    aliasGroups = (JSON.parse((raw.match(/\{[\s\S]*\}/) || ['{}'])[0]).groups || []).filter((g) => Array.isArray(g) && g.length >= 2);
...
const ranked = computeSalience({ lifts: liftRun.lifts, memoTexts, aliasGroups });
...
const out = { kind: 'salience', book: nrmT(book.title), lifts: liftRun.label, runAt: new Date().toISOString(), aliasGroups, ranked };
