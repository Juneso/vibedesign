// V11 — 책의 논지 전개 방식에 맞춘 위계 인제스트 (BKT-380)
//
...
//
// 사용: node eval/runHierV11.mjs "책1" "책2" "책3"
//        BOOKS="책1,책2" node eval/runHierV11.mjs
// 결과: runs/hier-v11-{N}.json + .md  (시리즈 hier-v11)
...
console.log(`[V11] 위계 ${MODEL} · planIngest ${INGEST_MODEL} · ${titles.length}권`);

// 항상 1부터 시작하면 부분 재실행이 기존 결과를 덮어쓴다 — 기존 번호 다음부터.
const { readdir } = await import('node:fs/promises');
const existing = (await readdir(resolve(__dir, 'runs'))).map((f) => f.match(/^hier-v11-(\d+)\.json$/)).filter(Boolean).map((m) => +m[1]);
...
for (const t of titles) {
  ...
  try {
    ...
    const r = await runHierIngest({
      book, memos, llm, embedFn, variant: 'v11',
    ...
    let md = `# ${N(b.title)} — V11 (관계 축 기반)\n\n`;
    md += `- 핵심 개념·관계 축: **${r.mode ? `${r.mode.mode} (확신 ${r.mode.confidence})` : '판정 실패'}**\n`;
