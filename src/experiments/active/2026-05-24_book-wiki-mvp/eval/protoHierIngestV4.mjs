...
// ─── Phase 2: 단일 군집화 패스 (전체 leaf를 한 번에, 숨은 목차 참고) ──
// 반복 지역 묶기(v3) 대신, 모든 leaf를 한꺼번에 보고 일관된 2단 구조를 1회 생성.
{
  const leaves = concepts(); // phase1 결과는 전부 root 직속 leaf
  const prompt = `[숨은 척추 — 참고만, 이 제목을 노드로 복붙하지 마라] 책 "${book.title}" 목차 흐름: ${book.toc.join(' · ')}

아래는 한 독자가 이 책에서 수집한 개념들이다. 책의 실제 주제 구조(숨은 목차)를 참고해 **상위 테마 몇 개로 묶어** 2단 위계를 만들어라.
- 상위 테마 수는 개념들이 자연히 나뉘는 만큼(보통 3~6개). 억지로 다 묶지 마라.
- 테마 이름 = 묶인 개념들을 아우르는 발생적 이름(목차 제목 복붙·자식명 동일 금지).
- 어디에도 안 어울리는 개념은 themeName을 빈 문자열("")로 둬서 최상위에 남겨라.
- 한 테마가 너무 커지면(6개+) 그 안에서 한 번 더 하위 묶음 허용.

개념:
${leaves.map((k) => `${k.id}: ${k.title} — ${k.gloss || ''}`).join('\n')}

출력 JSON: {"themes":[{"name":"상위 테마(빈문자열=최상위 유지)","children":[{"id":"n?","sub":"선택: 하위 묶음명 또는 빈문자열"}]}]}`;
  const raw = await llm({ system: SYS, user: prompt, temperature: 0.1 });
  let out; try { out = JSON.parse(raw); } catch { out = { themes: [] }; }
  const subCache = new Map(); // `${themeId}|${subName}` -> node
  for (const t of out.themes || []) {
    let themeNode = root;
    if (t.name && t.name.trim()) {
      themeNode = addConcept(t.name.trim(), root.id, null, '테마');
      log.push(`[군집] 상위 테마 "${t.name}"(${themeNode.id})`);
    }
    for (const ch of t.children || []) {
      const leaf = nodes.get(ch.id); if (!leaf || leaf.kind !== 'concept') continue;
      let parent = themeNode;
      if (ch.sub && ch.sub.trim()) {
        const key = `${themeNode.id}|${ch.sub.trim()}`;
        if (!subCache.has(key)) { const s = addConcept(ch.sub.trim(), themeNode.id, null, '하위 묶음'); subCache.set(key, s); log.push(`[군집]   하위 묶음 "${ch.sub}"(${s.id}) ⊂ ${themeNode.title}`); }
        parent = subCache.get(key);
      }
      leaf.parentId = parent.id; leaf.level = parent.level + 1;
    }
  }
}
